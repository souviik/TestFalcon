Feature: CoinGecko Bitcoin API Testing
  ...
  @Regression @APITest @CoinGecko @device_Window_11 @author_TestAutomation
  ...
  @Regression @APITest @device_Window_11 @author_TestAutomation
  Scenario: Verify Bitcoin API response performance and data integrity
    Given User sends GET request to CoinGecko Bitcoin API endpoint "https://api.coingecko.com/api/v3/coins/bitcoin"
    When User receives the API response
    Then User should verify the response status code is 200
    ...
    And User should verify Bitcoin coin ID is "bitcoin"
    And User should verify Bitcoin symbol is "btc"
    And User should verify Bitcoin name is "Bitcoin"
